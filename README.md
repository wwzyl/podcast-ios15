...
- 无文稿节目可使用 whisper.cpp + `ggml-base-q5_1` 在设备上离线生成逐句稿（模型首次按需下载）
...
如果 RSS 单集包含 `podcast:transcript`，App 会自动载入。没有文稿时可点“自动生成逐句稿”，也可在“更多”页导入对应的 SRT/VTT/JSON。Whisper 模型首次使用时下载并保存在 App Support 中，约 60 MB；生成结果会缓存，之后不需要重新转写。
