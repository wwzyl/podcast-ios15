...
1.5.0 完全移除云端转录实现，恢复为仅使用离线 Whisper。整集音频不再写入 `Application Support/EpisodeAudio` 子目录，而是分块复制到当前 App 的系统 Caches 根目录后原子落盘；旧后台任务携带的目标路径会被忽略，从而避开更新、重装或文件保护导致的目录权限错误。音频缓存和转录缓存互相独立，转录失败不会删除已下载音频。
